...
- [x] `[all]` Version matches the git tag (2026-08-31 — pyproject `1.0.0` == tag `v1.0.0` == `npm/package.json` `1.0.0` == `_FALLBACK_VERSION`. Enforced mechanically, not by eye: `release.yml` fails the release if `$TAG != $PKG` or `$PKG != $NPM`, and `verify.py --installed` refuses when installed metadata disagrees with pyproject. **This item was skipped as "no git tag exists yet ... nothing to compare the manifest's 0.1.0 against" through four releases** — v0.2.0, v0.2.1, v0.3.0, v0.4.0 — and it is the exact check that would have caught the stale editable dist-info that broke a release twice)
