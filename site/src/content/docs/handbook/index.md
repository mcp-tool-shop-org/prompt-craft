...
| | |
|---|---|
| The core | **322 tests passing** (counted 2026-08-18), GPU-free and deterministic — it runs anywhere, against a mock generator and verifier |
...
| SDXL conditioning | ControlNet OpenPose, IP-Adapter, **LoRA**, and regional inpaint are **wired and fake-torch tested**. Local `generate()` on a 5090 has not been run. InstantID still refuses |
| Flux encoder | Fill inpaint is wired. Pose / IP-Adapter / LoRA stay refused. `method=reference` writes the Cloud recipe and will not pretend Kontext ran locally |
