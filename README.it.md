...
**v1.0.0: le INTERFACCE sono stabili. Le immagini non sono ancora complete e questo documento non pretende il contrario.**

Qui, `1.0.0` rappresenta un'affermazione relativa alla CLI, ai percorsi di importazione, ai codici di uscita e ai due formati su disco, elencati in [STABILITY.md](STABILITY.md), insieme a ciò che è stato escluso intenzionalmente. Non si tratta di un'affermazione secondo cui l'immagine viene visualizzata correttamente sui pixel. Le lacune indicate di seguito sono reali e migliorano nelle versioni successive; ciò che smette di cambiare è la superficie su cui si basa il processo.

| | |
|---|---|
| Nucleo | **394 test superati** (conteggio del 2026-08-18), esecuzione senza GPU, risultati deterministici. `verify` esegue controlli sulla coerenza delle versioni, analisi statica del codice, controllo dei tipi, la suite di test completa, la stessa suite nuovamente sotto `-O` e la creazione di un pacchetto; quindi **indica cosa non ha controllato**. Esegue l'analisi statica del codice e il controllo dei tipi su se stesso, con vincoli imposti da un test in modo che gli obiettivi non possano essere modificati. |
...
- La regola secondo cui un modello generativo non può mai essere il proprio controllore è stata presentata come se fosse stata stabilita da uno studio. Le prove a sostegno sono **convergenti piuttosto che dirette**: l'analisi discriminativa sì/no si rivela misurabilmente più stabile rispetto alla generazione di didascalie aperte, i modelli non possono correggersi in modo affidabile senza feedback esterno e il riconoscimento automatico traccia le distorsioni delle preferenze. Nessuno studio singolo ha effettuato un confronto diretto. La regola è valida; la certezza è stata esagerata.
- Il condizionamento è stato inizialmente descritto come non implementato, poi come non ancora implementato. SDXL ora **legge** i riferimenti assemblati nel codice. È stata eseguita una prova locale su questa macchina con `generate()`. L'esecuzione e l'applicazione non sono la stessa cosa dell'immagine che viene visualizzata correttamente sui pixel.
- `verify` è stato descritto elencando le fasi di esecuzione, il che ha portato a interpretare che un `verify.py` verde corrisponda a una CI verde. In realtà non è così: l'analisi delle dipendenze viene eseguita come fase separata della CI. Il controllore ora stampa ciò che **non** ha controllato e `--audit` esiste per quando si desidera eseguire tale fase localmente. Nella stessa esecuzione: il controllore non aveva mai eseguito l'analisi statica del codice o il controllo dei tipi sul proprio codice sorgente, e l'insieme di regole per l'analisi statica è stato ereditato dalla versione dello strumento utilizzata anziché essere dichiarato. Entrambi i problemi sono stati risolti nella versione 0.4.0. Un controllo che sembra attivo ma fa meno di quanto sembri è esattamente il tipo di errore che questo progetto si propone di individuare, ed era presente negli strumenti utilizzati.

## Requisiti

| | |
|---|---|
| Python | **3.11+** (la CI esegue le versioni 3.11 e 3.13 sul core + `[dev]`. L'extra `[image]` non è garantito per la versione 3.11). |
