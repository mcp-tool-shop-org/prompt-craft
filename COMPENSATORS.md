...
|---|--------|------------------------|---------------------|-------|
| R0 | **disposition-write** -- record a human's decision about an escalated receipt as a new file under `records/dispositions/` | delete the disposition file at the path `record_disposition()` returned (no named fn: the write is a single new file that references the receipt and edits nothing, so the undo is the file's removal; `disposition_paths()` re-lists what remains) | Receipt unchanged (it was never edited); the decision record gone; `pcraft resolve` can re-record | Operator via `pcraft resolve`; the write refuses unless the `disposition-write` compensator is registered |
