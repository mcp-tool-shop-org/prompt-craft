...
**v0.4.0 — le noyau est opérationnel et la porte signale désormais honnêtement son état. Le conditionnement SDXL est assemblé dans le code. Une exécution locale avec une carte 5090 `generate()` a été effectuée. Un script Cloud a été exécuté en direct.**

| | |
|---|---|
| Cœur du système | **359 tests réussis** (comptés le 2026-08-18), sans utilisation du GPU, déterministe. `verify` exécute la vérification de cohérence des versions, l’analyse statique, la vérification des types, la suite de tests, puis la suite de tests à nouveau sous `-O`, et effectue une compilation du package — puis **indique ce qu’il n’a pas vérifié**. Il effectue une analyse statique et une vérification des types sur lui-même, et un test est utilisé pour empêcher les cibles de se restreindre. |
...
- Les seuils à trois zones ont été décrits comme *calibrés par rapport à un ensemble de données de référence étiqueté par des humains*. Ce n’est pas le cas. Il s’agit de valeurs par défaut.
- La règle selon laquelle un modèle génératif ne peut jamais servir de porte a été énoncée comme si une étude l’avait établie. Les preuves à l’appui sont **plutôt convergentes que directes** : les sondages discriminatoires oui/non se révèlent être sensiblement plus stables que la génération de légendes ouverte, les modèles ne peuvent pas s’auto-corriger de manière fiable sans rétroaction externe et la reconnaissance de soi suit un biais d’autopréférence. Aucune étude unique n’effectue une comparaison directe. La règle est valable ; le degré de certitude a été exagéré.
- Le conditionnement a été décrit comme non lu, puis comme non implémenté. SDXL **lit** désormais les références assemblées dans le code. Une exécution locale en direct avec `generate()` sur cette machine a maintenant été effectuée. Le fait que ce soit « câblé et appliqué » n’est pas la même chose que l’obtention du résultat final dans les pixels.
- `verify` a été décrit en énumérant les étapes qu’il exécute, ce qui a suggéré qu’un `verify.py` vert est un CI vert. Ce n’est pas le cas : l’audit des dépendances s’exécute comme une étape distincte du CI. La porte affiche désormais ce qu’elle n’a **pas** vérifié, et `--audit` existe pour lorsque vous souhaitez exécuter cette étape localement. Dans la même passe : la porte n’avait jamais effectué d’analyse statique ou de vérification des types sur son propre code source, et l’ensemble des règles d’analyse statique était hérité de la version de l’outil utilisée plutôt que déclaré. Les deux problèmes sont corrigés dans la version 0.4.0. Une vérification qui semble être effectuée en direct alors qu’elle fait moins de ce qu’elle prétend faire est précisément le type d’erreur que ce projet vise à détecter, et elle était présente dans les outils.
