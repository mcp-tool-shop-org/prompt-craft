...
**v1.0.0 — les INTERFACES sont stables. Les images ne sont pas terminées, et ce document ne prétend pas le contraire.**

Un `1.0.0` ici est une affirmation concernant l’interface en ligne de commande (CLI), les chemins d’importation, les codes de sortie et les deux formats sur disque — énumérés dans [STABILITY.md](STABILITY.md), ainsi que ce qui est délibérément exclu. Il ne s’agit pas d’une affirmation selon laquelle l’image se superpose parfaitement aux pixels. Les écarts ci-dessous sont réels et s’améliorent dans les versions mineures ; ce qui cesse de bouger, c’est la surface sur laquelle vous travaillez.

| | |
|---|---|
| Cœur du système | **394 tests réussis** (comptés le 2026-08-18), sans GPU, déterministe. `verify` exécute des vérifications de cohérence de version, des analyses de code, des vérifications de type, la suite de tests, puis à nouveau la suite sous `-O`, et effectue une compilation du package — puis **indique ce qu’il n’a pas vérifié**. Il analyse et vérifie le type de son propre code, en utilisant un test pour empêcher les cibles de se restreindre. |
...
- Les trois seuils de zone ont été décrits comme *calibrés par rapport à un ensemble de données étiqueté par des humains*. Ce n’est pas le cas. Il s’agit de valeurs par défaut.
- La règle selon laquelle un modèle génératif ne peut jamais être son propre contrôleur a été énoncée comme si une étude l’avait établie. Les preuves à l’appui sont **plutôt convergentes que directes** — les sondages discriminatoires oui/non se révèlent mesurablement plus stables que la génération de légendes ouverte, les modèles ne peuvent pas corriger de manière fiable leurs propres erreurs sans rétroaction externe et la reconnaissance de soi suit un biais de préférence personnelle. Aucune étude unique n’effectue une comparaison directe. La règle est valable ; le niveau de certitude a été exagéré.
- Le conditionnement a d’abord été décrit comme non lu, puis comme non implémenté. SDXL **lit** désormais les références assemblées dans le code. Un `generate()` local en direct sur cette machine a maintenant été exécuté. Le fait que ce soit « câblé et appliqué » n’est pas la même chose que l’image qui se superpose parfaitement aux pixels.
- `verify` a été décrit en énumérant les étapes qu’il exécute, ce qui a conduit à penser qu’un `verify.py` vert est un CI vert. Ce n’est pas le cas ; l’audit des dépendances s’exécute comme une étape CI distincte. Le contrôleur affiche désormais ce qu’il n’a **pas** vérifié, et `--audit` existe pour lorsque vous souhaitez exécuter cette étape localement. Dans la même passe : le contrôleur n’avait jamais analysé ou vérifié le type de son propre code, et l’ensemble des règles d’analyse a été hérité de la version de l’outil qui s’est résolue plutôt que déclaré. Les deux ont été corrigés dans la version 0.4.0. Une vérification qui donne l’impression d’être en direct alors qu’elle fait moins qu’il n’y paraît est exactement le type d’erreur que ce projet vise à détecter, et c’était présent dans les outils.
