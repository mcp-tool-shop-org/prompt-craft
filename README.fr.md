...
**Les différents modèles sont à configurer par l’utilisateur.** Le vérificateur de palette déterministe fonctionne à partir d’une installation de base (il inclut sa propre bibliothèque standard de lecture de fichiers PNG, ce qui évite d’avoir besoin de Pillow). Les vérificateurs pour les différents modèles nécessitent deux paquets, **sans déclarations supplémentaires** : `t2v-metrics` (PyPI) pour la famille VQA, et `ai-eyes-mcp` (un paquet distinct mcp-tool-shop, qui n’est pas sur PyPI) pour l’écran SigLIP. Ils ne sont pas spécifiés dans `pyproject.toml`, car aucune version n’a été testée en termes d’intégration avec cette configuration. Spécifier une version impliquerait une compatibilité qui n’a pas été vérifiée. `pcraft doctor` les signale tous les deux sous « modèle », et une porte dont les atomes du modèle sont nommés SKIPPED les mentionne dans son refus ; SKIPPED n’est jamais compté silencieusement comme une réussite.
...
**v1.0.1 — les INTERFACES sont stables. Les images ne sont pas terminées, et ce document ne prétend pas le contraire.**

Ici, `1.0.0` représente une affirmation concernant l’interface en ligne de commande, les chemins d’importation, les codes de sortie et les deux formats de fichiers — énumérés dans [STABILITY.md](STABILITY.md), ainsi que ce qui est délibérément exclu. Il ne s’agit pas d’une affirmation selon laquelle l’image s’affiche correctement dans les pixels. Les lacunes ci-dessous sont réelles et s’améliorent dans les versions mineures ; ce qui cesse de changer est la surface sur laquelle vous travaillez.

| | |
|---|---|
| Cœur du système | **1 162 tests réussis** (comptés le 2023-08-31), sans utilisation de GPU, déterministe. `verify` exécute la vérification de la cohérence des versions, l’analyse de code, la vérification des types, la suite de tests, puis la suite de tests à nouveau sous `-O`, et enfin la création d’un paquet — puis **indique ce qu’il n’a pas vérifié**. Il effectue une analyse de code et une vérification des types sur lui-même, et un test fixe les cibles afin qu’elles ne puissent pas être réduites. |
...
- Les seuils à trois zones ont été décrits comme *calibrés par rapport à un ensemble de données de référence étiqueté par des humains*. Ce n’est pas le cas. Ce sont des valeurs par défaut.
- La règle selon laquelle un modèle génératif n’est jamais sa propre porte a été énoncée comme si une étude l’avait établie. Les preuves à l’appui sont **plutôt convergentes que directes** : les sondages discriminatoires oui/non sont mesurablement plus stables que la génération de légendes ouverte, les modèles ne peuvent pas se corriger de manière fiable sans rétroaction externe, et la reconnaissance de soi suit les biais de préférence de soi. Aucune étude unique ne compare directement les deux. La règle est valable ; la certitude a été exagérée.
- Le conditionnement a été décrit comme non lu, puis comme non implémenté. SDXL lit maintenant les références assemblées dans le code. Un `generate()` local en direct sur cette machine a maintenant été exécuté. Le fait qu’il soit câblé et appliqué n’est pas la même chose que l’image qui s’affiche correctement dans les pixels.
- `verify` a été décrit en énumérant les étapes qu’il exécute, ce qui a suggéré qu’un `verify.py` vert est un CI vert. Ce n’est pas le cas : l’audit des dépendances s’exécute comme une étape CI distincte. La porte affiche maintenant ce qu’elle n’a **pas** vérifié, et `--audit` existe pour lorsque vous souhaitez exécuter cette étape localement. Dans la même passe : la porte n’avait jamais effectué d’analyse de code ou de vérification des types sur son propre code source, et l’ensemble des règles d’analyse de code était hérité de la version de l’outil qui était résolue, plutôt que déclaré. Les deux ont été corrigés dans la version 0.4.0. Une vérification qui donne l’impression d’être en direct tout en faisant moins qu’elle ne le semble est exactement le type d’erreur que ce projet vise à détecter, et c’était dans les outils.
