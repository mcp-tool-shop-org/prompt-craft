...
**v0.4.0 —核心功能已实现，并且现在闸门会如实报告自身状态。SDXL的条件设置已在代码中完成。一台本地5090`generate()`已经运行。一个云端配方已进行实时测试。**

| | |
|---|---|
| 核心 | **359项测试通过**（统计于2026-08-18），无需GPU，结果可重复。`verify`运行版本一致性检查、代码风格检查、类型检查、完整的测试套件，以及在`-O`下再次运行完整的测试套件，并进行软件包构建——然后**说明它没有检查哪些内容**。它会对自己进行代码风格和类型检查，并通过一个测试固定下来，以防止目标范围缩小。 |
...
- 三个阈值被描述为*经过与人工标注的保留数据集的校准*。但实际上并非如此。它们是默认值。
- 关于生成模型永远不能作为自身的闸门这一规则，其表述方式似乎暗示有研究已经证实了这一点。支持证据是**趋同而非直接的**——可测量的二元（是/否）投票比开放式的文本描述更稳定，模型无法在没有外部反馈的情况下可靠地进行自我纠正，并且自我识别会反映出自我偏好偏差。没有任何一项单独的研究进行了直接对比。该规则是合理的；但对其确定性的描述过于夸大。
- 最初，条件设置被描述为未读取，然后又被描述为未实现。现在，SDXL**读取**代码中已完成的引用。一台本地`generate()`已经在本机上运行。虽然已经连接并应用了，但这并不等同于最终结果呈现在像素中。
- `verify`通过列出它运行的各个环节来描述，这可能会让人误以为绿色的`verify.py`是一个绿色的CI（持续集成）。但事实并非如此——依赖项审计作为单独的CI步骤进行。闸门现在会打印它**没有**检查的内容，并且存在`--audit`，以便在您需要本地执行该环节时使用。在同一过程中：闸门从未对其自身源代码进行代码风格或类型检查，并且代码风格规则集是从所使用的工具版本中继承而来，而不是声明的。两者都在v0.4.0中得到了修复。一个看起来正在运行，但实际上执行的操作少于其表面上显示的内容的检查，正是这个项目旨在捕捉的错误，而该错误就存在于工具中。
