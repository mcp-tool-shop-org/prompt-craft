...
| Condicionamiento SDXL | ControlNet OpenPose, IP-Adapter (`method=ip_adapter`), **LoRA (`method=lora`)** y el retoque regional están **conectados y cubiertos por pruebas de fake-torch**. No se ha ejecutado la versión local `generate()` en una tarjeta 5090. InstantID sigue sin funcionar. |
